...
pub async fn handle_command(
    ...
    match name {
        ...
        "clear" | "new" | "reset" => core::clear(ctx, args).await,
        ...
        "network" => parity::network(ctx, args).await,
        ...
        "continue" => chat::resume_cmd(ctx, args).await,
        "quit" => core::exit(ctx, args).await,
        "hist" => parity::history(ctx, args).await,
        "mem" => memory_wrapper(ctx, args).await,
        "mstore" => memory_wrapper(ctx, args).await,
        "allowed-tools" => permissions::run(ctx, args).await,
        "watch-pr" => parity::subscribe_pr(ctx, args).await,
        "api" => provider_wrapper(ctx, args).await,
        "app" => parity::desktop(ctx, args).await,
        "ios" => parity::mobile(ctx, args).await,
        "android" => parity::mobile(ctx, args).await,
        "who" => parity::peers(ctx, args).await,
        "cron" => parity::schedule(ctx, args).await,
        "rc" => parity::remote_control(ctx, args).await,
        "rcs" => parity::remote_control(ctx, args).await,
        "color" => config::theme(ctx, args).await,
        "detach" => parity::detach(ctx, args).await,
        "schedule" => parity::schedule(ctx, args).await,
        "triggers" => parity::triggers(ctx, args).await,
        "commit" => git_wrapper(args).await,
        "commit-push-pr" => parity::commit_push_pr(ctx, args).await,
        "remote-env" => parity::remote_env(ctx, args).await,
        "pipes" => parity::pipes(ctx, args).await,
        "pipe-status" => parity::pipe_status(ctx, args).await,
        "break-cache" => parity::break_cache(ctx, args).await,
        "autofix-pr" => parity::autofix_pr(ctx, args).await,
        "thinkback-play" => parity::thinkback_play(ctx, args).await,
        "force-snip" => parity::force_snip(ctx, args).await,
        "remote-control-server" => parity::remote_control_server(ctx, args).await,
...
async fn index_cmd(ctx: CommandContext<'_>, args: Vec<String>) -> CommandResult {
        ...
        .filter(|e| e.file_type().is_some_and(|t| t.is_file()))
    {
        ...
        if !crate::core::tools::codebase_search::is_indexable_ext(ext) {
            continue;
        }
