...
impl MessageFactory {
    /// 创建用户消息
    pub fn create_user_message(content: &str, options: UserMessageOptions) -> StarMessage {
    ...
    pub fn create_model_switch_breadcrumbs(
        ...
        vec![
            ...
            StarMessage::user(&format!("/model {}", model_arg)),
        ]
    }
...
/// 检查是否是仅工具调用响应
pub fn is_tool_call_only_response(
    content: &Option<String>,
    tool_calls: &Option<Vec<StarToolCall>>,
) -> bool {
    let has_content = content.as_ref().map_or(false, |c| !c.trim().is_empty());
    let has_tools = tool_calls.as_ref().map_or(false, |t| !t.is_empty());
    !has_content && has_tools
...
mod tests {
    ...
    fn test_reject_message() {
        assert!(SyntheticMessageDetector::is_reject_message(
            constants::REJECT_MESSAGE
        ));
        assert!(SyntheticMessageDetector::is_reject_message(&format!(
            "{} user said stop",
            constants::REJECT_MESSAGE
        )));
    }
