...
| Search by meaning | `CodebaseSearch` | — |
...
## Workflow
1. **Locate** → `CodebaseSearch` for conceptual/functional questions ("how is X handled", "where does Y live"); `Grep` for exact symbols/strings; `Glob` for filename patterns.
