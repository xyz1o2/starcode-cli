...
pub(crate) fn build_semantic_search_tool_call(user_input: &str, turn: i32) -> StarToolCall {
    build_tool_call("SemanticSearch", &serde_json::json!({"query": user_input, "turn": turn}))
}

pub(crate) fn build_project_map_tool_call(user_input: &str, turn: i32) -> StarToolCall {
    build_tool_call("ProjectMap", &serde_json::json!({"query": user_input, "turn": turn}))
}
...
pub(crate) fn build_json_fallback_prompt(content: &str, active_tools: &std::collections::HashSet<String>) -> String {
    let tools_str: Vec<&str> = active_tools.iter().map(|s| s.as_str()).collect();
    let tools_joined = tools_str.join(", ");
    format!("Please respond with valid JSON. Active tools: {}\n\n{}", tools_joined, content)
}
...
fn build_skill_tool_call(skill: &str, user_input: &str, turn: i32) -> StarToolCall {
    build_tool_call("skill", &serde_json::json!({
        "skill": skill,
        "task": user_input,
        "turn": turn
    }))
...
fn build_tool_call(name: &str, args: &serde_json::Value) -> StarToolCall {
    StarToolCall {
        id: format!("call_{}_{}", name, uuid::Uuid::new_v4().to_string().replace('-', "")[..8].to_string()),
